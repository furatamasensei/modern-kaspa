...
export default class Dag {
  ...
  private currentScale: number = 0.75
  ...
  private blockInformationChangedListener: (
  ...
  initialize = (canvas: HTMLCanvasElement) => {
    ...
    this.timelineContainer.setDAAScoreClickedListener(
      this.handleDAAScoreClicked,
    )
    ...
    resolveDataSource().then((dataSource) => {
      ...
      this.tickIntervalInMilliseconds =
        dataSource.getTickIntervalInMilliseconds()
  ...
  private scheduleNextTick = () => {
    this.currentTickId = window.setTimeout(
      this.tick,
      this.tickIntervalInMilliseconds,
    )
  }
  ...
  private trackTargetHeight = async () => {
    ...
    const [startHeight, endHeight] =
      this.timelineContainer!.getVisibleHeightRange(targetHeight)
    const blocksAndEdgesAndHeightGroups =
      await this.dataSource!.getBlocksBetweenHeights(startHeight, endHeight)
    ...
    this.timelineContainer!.setTargetHeight(
      targetHeight,
      blocksAndEdgesAndHeightGroups,
    )
    this.timelineContainer!.setBlocksAndEdgesAndHeightGroups(
      blocksAndEdgesAndHeightGroups,
    )
  }
  ...
  private trackTargetDAAScore = async () => {
    ...
    const heightDifference =
      this.timelineContainer!.getMaxBlockAmountOnHalfTheScreen()
    const blocksAndEdgesAndHeightGroups =
      await this.dataSource!.getBlockDAAScore(targetDAAScore, heightDifference)
    this.isFetchFailingListener(!blocksAndEdgesAndHeightGroups)
  ...
  private trackTargetHash = async () => {
    ...
    const heightDifference =
      this.timelineContainer!.getMaxBlockAmountOnHalfTheScreen()
    const blocksAndEdgesAndHeightGroups = await this.dataSource!.getBlockHash(
      targetHash,
      heightDifference,
    )
    this.isFetchFailingListener(!blocksAndEdgesAndHeightGroups)
    ...
    if (!targetBlock) {
      console.error(
        `Block ${targetHash} not found in blockHash response ${blocksAndEdgesAndHeightGroups}`,
      )
      return
    }
  ...
  private trackHead = async () => {
    ...
    const heightDifference =
      this.timelineContainer!.getVisibleSlotAmountAfterHalfTheScreen(
        theme.components.dag.headMinRightMargin,
      )
    const blocksAndEdgesAndHeightGroups = await this.dataSource!.getHead(
  ...
  private buildBlockInformation = async (
    block: Block,
  ): Promise<BlockInformation> => {
    let notFoundIds: number[] = []

    let [parentHashes, notFoundParentIds] = this.getCachedBlockHashes(
      block.parentIds,
    )
    ...
    if (block.selectedParentId) {
      const [selectedParentHashes, notFoundSelectedParentIds] =
        this.getCachedBlockHashes([block.selectedParentId])
      notFoundIds = notFoundIds.concat(notFoundSelectedParentIds)

      selectedParentHash = selectedParentHashes[0]
    }

    let [mergeSetRedHashes, notFoundMergeSetRedIds] = this.getCachedBlockHashes(
      block.mergeSetRedIds,
    )
    notFoundIds = notFoundIds.concat(notFoundMergeSetRedIds)

    let [mergeSetBlueHashes, notFoundMergeSetBlueIds] =
      this.getCachedBlockHashes(block.mergeSetBlueIds)
    notFoundIds = notFoundIds.concat(notFoundMergeSetBlueIds)

    const [childIds, selectedChildId] = getBlockChildIds(
      this.timelineContainer!.gettBlocksAndEdgesAndHeightGroups()!,
      block,
    )
    ...
    if (notFoundIds.length > 0) {
      const blockHashesByIds = await this.dataSource!.getBlockHashesByIds(
        notFoundIds.join(','),
      )
      if (blockHashesByIds) {
  ...
  setIsTrackingChangedListener = (
    isTrackingChangedListener: (isTracking: boolean) => void,
  ) => {
    this.isTrackingChangedListener = isTrackingChangedListener
  }

  setIsFetchFailingListener = (
    isFetchFailingListener: (isFailing: boolean) => void,
  ) => {
    this.isFetchFailingListener = isFetchFailingListener
  }

  setBlockInformationChangedListener = (
    BlockInformationChangedListener: (
      blockInformation: BlockInformation | null,
    ) => void,
  ) => {
  ...
  setAppConfigChangedListener = (
    AppConfigChangedListener: (appConfig: AppConfig | null) => void,
  ) => {
