...
export default class TimelineContainer extends PIXI.Container {
  ...
  private readonly heightKeysToHeightSprites: {
    [heightKey: string]: HeightSprite
  } = {}
  private readonly blockKeysToBlockSprites: {
    [blockKey: string]: BlockSprite
  } = {}
  private readonly edgeKeysToEdgeSprites: { [edgeKey: string]: EdgeSprite } = {}

  private currentBlocksAndEdgesAndHeightGroups: BlocksAndEdgesAndHeightGroups | null =
    null
  ...
  gettBlocksAndEdgesAndHeightGroups =
    (): BlocksAndEdgesAndHeightGroups | null =>
      this.currentBlocksAndEdgesAndHeightGroups

  setBlocksAndEdgesAndHeightGroups = (
    blocksAndEdgesAndHeightGroups: BlocksAndEdgesAndHeightGroups,
    targetBlock: Block | null = null,
  ) => {
    // Don't bother updating anything if there's nothing to update
    // noinspection JSSuspiciousNameCombination
    if (
      this.currentBlocksAndEdgesAndHeightGroups !== null &&
      areBlocksAndEdgesAndHeightGroupsEqual(
        this.currentBlocksAndEdgesAndHeightGroups,
        blocksAndEdgesAndHeightGroups,
      )
    ...
    const targetBlockKey = targetBlock
      ? this.buildBlockKey(targetBlock.id)
      : null
    ...
    Object.keys(heightKeysInBlocks)
      ...
      .forEach((heightKey) => {
        ...
        const heightDAAScore = getHeightGroupDAAScore(
          this.currentBlocksAndEdgesAndHeightGroups!,
          heightGroup.height,
        )
        const heightSprite = new HeightSprite(
          this.application,
          heightGroup.height,
          heightDAAScore,
        )
    ...
    Object.keys(heightKeysInBlocks).forEach((heightKey) => {
      const heightGroup = this.heightKeysToHeightGroups[heightKey]
    ...
    for (let edge of edges) {
      ...
      if (!this.edgeKeysToEdgeSprites[edgeKey]) {
        ...
        const edgeSprite = new EdgeSprite(
          this.application,
          edge.fromBlockId,
          edge.toBlockId,
        )
  ...
  private assignBlockToBlockSprite = (
    ...
    const [childIds] = getBlockChildIds(
      this.currentBlocksAndEdgesAndHeightGroups!,
      targetBlock,
    )
    const childBlockKeys = childIds.map((blockId) =>
      this.buildBlockKey(blockId),
    )
    ...
    const mergeSetRedBlockKeys = targetBlock.mergeSetRedIds.map((blockId) =>
      this.buildBlockKey(blockId),
    )
    const mergeSetBlueBlockKeys = targetBlock.mergeSetBlueIds.map((blockId) =>
      this.buildBlockKey(blockId),
    )

    const mergeSetBlockKeys = mergeSetRedBlockKeys.concat(mergeSetBlueBlockKeys)
    if (
      mergeSetBlockKeys.indexOf(blockKey) < 0 &&
  ...
  private assignEdgeToEdgeSprite = (
    ...
    isHighlightedParent =
      this.buildBlockKey(edge.fromBlockId) === targetBlockKey
    isHighlightedChild = this.buildBlockKey(edge.toBlockId) === targetBlockKey

    edgeSprite.setFullState(
      isInVirtualSelectedParentChain,
      isHighlightedParent,
      isHighlightedChild,
      isSelectedParent,
  ...
  recalculateTargetHeight = () => {
    if (this.currentBlocksAndEdgesAndHeightGroups !== null) {
      this.targetHeight = getDAAScoreGroupHeight(
        this.currentBlocksAndEdgesAndHeightGroups,
        this.targetDAAScore,
      )
    }
  ...
  private recalculateHeightSpritePositions = () => {
    ...
    Object.values(this.heightKeysToHeightSprites).forEach((sprite) => {
  ...
  private recalculateBlockSpritePositions = (animate: boolean) => {
    ...
    Object.entries(this.blockKeysToBlocks).forEach(([blockKey, block]) => {
      const blockSprite = this.blockKeysToBlockSprites[blockKey]
      ...
      const heightGroup = this.heightKeysToHeightGroups[heightKey]
      blockSprite.x = this.calculateBlockSpriteX(
        block.height,
        blockSize,
        marginX,
      )
  ...
  private recalculateEdgeSpritePositions = (animate: boolean) => {
    ...
    Object.entries(this.edgeKeysToEdges).forEach(([edgeKey, edge]) => {
      const edgeSprite = this.edgeKeysToEdgeSprites[edgeKey]
      ...
      Tween.get(tween, { onChange: onChange }).to(
        { fromY: fromY, toY: toY },
        500,
        Ease.quadOut,
      )
  ...
  private calculateBlockSize = (rendererHeight: number): number => {
    return Math.floor(
      (rendererHeight * this.scaleGetter()) /
        theme.components.timeline.maxBlocksPerHeight,
    )
  }
  ...
  private moveTimelineContainer = () => {
    ...
    const blockSpriteXForTargetHeight = this.calculateBlockSpriteX(
      this.targetHeight,
      blockSize,
      marginX,
    )
  ...
  getMaxBlockAmountOnHalfTheScreen = (): number => {
    ...
    return (
      ...
      theme.components.timeline.visibleHeightRangePadding
    )
  }
  ...
  getVisibleSlotAmountAfterHalfTheScreen = (rightMargin: number): number => {
    ...
    const widthBetweenCenterAndRightMargin = Math.max(
      0,
  ...
  setTargetHeight = (
    targetHeight: number,
    newData?: BlocksAndEdgesAndHeightGroups,
  ) => {
  ...
  setDAAScoreClickedListener = (
    daaScoreClickedListener: (daaScore: number) => void,
  ) => {
