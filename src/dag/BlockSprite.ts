...
const blockTexture = (
  ...
  if (!blockTextures[key]) {
    ...
    graphics.drawRoundedRect(
      ...
      blockSize,
      blockSize,
      theme.scale(theme.components.block.roundingRadius, blockSize),
    )
    graphics.endFill()

    const textureOptions: PIXI.IGenerateTextureOptions = {
      scaleMode: PIXI.SCALE_MODES.LINEAR,
      resolution: resolution,
    }
    blockTextures[key] = application.renderer.generateTexture(
      graphics,
      textureOptions,
    )
  }
...
export default class BlockSprite extends PIXI.Container {
  ...
  constructor(application: PIXI.Application, block: Block) {
    ...
    this.scale.set(
      theme.components.block.scale.default,
      theme.components.block.scale.default,
    )
  }
  ...
  private buildSprite = (): PIXI.Sprite => {
    ...
    sprite.on('pointerover', () => {
      Tween.get(this.scale).to(
        {
          x: theme.components.block.scale.hover,
          y: theme.components.block.scale.hover,
        },
        200,
        Ease.quadOut,
      )
    ...
    sprite.on('pointerout', () => {
      Tween.get(this.scale).to(
        {
          x: theme.components.block.scale.default,
          y: theme.components.block.scale.default,
        },
        200,
        Ease.quadOut,
      )
  ...
  private buildText = (blockSize: number): PIXI.Text => {
    const nominalFontSize =
      blockSize * theme.components.block.text.multiplier.size * 2
    const textLines =
      nominalFontSize <= theme.components.block.text.minFontSize - 2
        ? Math.max(
            1,
            Math.min(
              Math.floor(
                nominalFontSize / theme.components.block.text.minFontSize,
              ),
        ...
        : Math.max(
            ...
            Math.min(
              Math.ceil(
                (nominalFontSize + 2) / theme.components.block.text.maxFontSize,
              ),
              theme.components.block.text.maxTextLines,
            ),
          )
    const style = new PIXI.TextStyle({
      ...
      fontSize:
        Math.floor(
          Math.min(
            nominalFontSize / textLines,
            theme.components.block.text.maxFontSize,
          ) * 4.0,
        ) / 4.0,
    ...
    const lastBlockHashCharacters = this.block.blockHash
      .substring(blockHashLength - lastCharactersLength)
      .toUpperCase()
    const displayHash = chunkSubstr(lastBlockHashCharacters, lineLength).join(
      '\n',
    )
  ...
  private buildHighlight = (): PIXI.Graphics => {
    ...
    const highlightSize =
      this.blockSize + theme.scale(blockHighlight.offset, this.blockSize)
    const highlightRoundingRadius = theme.scale(
      theme.components.block.roundingRadius + blockHighlight.offset / 2,
      this.blockSize,
    )
  ...
  setSize = (blockSize: number) => {
    if (!this.currentSprite.texture || this.blockSize !== blockSize) {
      ...
      this.currentSprite.texture = blockTexture(
        this.application,
        blockSize,
        this.blockColor,
      )
  ...
  setHighlighted = (
    ...
    if (
      ...
      if (isHighlighted) {
        Tween.get(this.currentHighlight).to({ alpha: 1.0 }, 300)
      }

      const toAlpha = this.isHighlighted ? blockHighlight.alpha : 0.0
      if (toAlpha !== this.highlightContainer.alpha) {
        Tween.get(this.highlightContainer).to({ alpha: toAlpha }, 300)
      }
    }
  }
  ...
  private getHighlightFrame = (): HighlightFrame => {
    return this.hasFocus
      ? theme.components.block.focus
      : theme.components.block.highlight
  }
  ...
  static getRealBlockSize = (blockSize: number): number => {
    ...
    return (
      (blockSize +
        theme.scale(
          theme.components.block.blue.border.width / 2.0,
          blockSize,
        )) *
      theme.components.block.scale.default
    )
  ...
  static clampVectorToBounds = (
    ...
    const roundingRadius = theme.scale(
      theme.components.block.roundingRadius,
      blockSize,
    )
