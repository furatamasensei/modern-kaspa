...
const heightTexture = (
  ...
  if (!heightTextures[key]) {
    ...
    heightTextures[key] = application.renderer.generateTexture(
      graphics,
      textureOptions,
    )
  }
...
export default class HeightSprite extends PIXI.Container {
  ...
  private buildText = (spriteHeight: number, blockSize: number): PIXI.Text => {
    const fontSize = Math.round(
      Math.max(
        theme.components.height.text.minFontSize,
        Math.min(
          blockSize * theme.components.height.text.multiplier.size,
    ...
    const bottomMargin = Math.round(
      Math.max(
        theme.components.height.text.minBottomMargin,
        Math.min(
          blockSize * theme.components.height.text.multiplier.bottomMargin,
    ...
    const spriteMarginXMult =
      (theme.components.height.text.multiplier.marginX * blockSize) /
      this.spriteWidth
    const language = navigator.language || 'en-US'
    const text = new PIXI.Text(
      this.getTextValue().toLocaleString(language),
      style,
    )
    text.anchor.set(0.5, 0.5)
    text.tint = theme.components.height.color.contrastText
    text.y = spriteHeight / 2 - bottomMargin
    const textSpriteSpread = Math.ceil(
      text.width / this.spriteWidth + spriteMarginXMult,
    )
  ...
  setSize = (width: number, height: number, blockSize: number) => {
    if (
      ...
      this.spriteWidth = width
      this.spriteHeight = height
      this.currentSprite.texture = heightTexture(
        this.application,
        width,
        height,
      )
