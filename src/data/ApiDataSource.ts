...
export default class ApiDataSource implements DataSource {
  ...
  getBlocksBetweenHeights = (
    startHeight: number,
    endHeight: number,
  ): Promise<BlocksAndEdgesAndHeightGroups | void> => {
    return this.fetch(
      `${apiAddress}/blocksBetweenHeights?startHeight=${startHeight}&endHeight=${endHeight}`,
    )
  }
  ...
  getBlockHash = (
    ...
    return this.fetch(
      `${apiAddress}/blockHash?blockHash=${targetHash}&heightDifference=${heightDifference}`,
    )
  }
  ...
  getBlockDAAScore = (
    ...
    return this.fetch(
      `${apiAddress}/blockDAAScore?blockDAAScore=${targetDAAScore}&heightDifference=${heightDifference}`,
    )
  }
  ...
  getAppConfig = (): Promise<AppConfig | void> => {
    return this.fetch<AppConfig>(`${apiAddress}/appConfig`).then((config) => {
      if (config) {
